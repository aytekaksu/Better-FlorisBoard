...
class AutoCorrectionRevertTest :
    FunSpec({
        ...
        test("expected content queue retains every item under concurrent pushes and pops") {
            val queue = AbstractEditorInstance.ExpectedContentQueue()
            val expected = List(256) { content(text = "$it", selection = EditorRange.cursor(1)) }

            coroutineScope {
                expected.chunked(32).map { batch ->
                    async(Dispatchers.Default) { batch.forEach(queue::push) }
                }.awaitAll()
            }
            val popped = coroutineScope {
                expected.map {
                    async(Dispatchers.Default) {
                        queue.popUntilOrNull(EditorRange.cursor(1), EditorRange.Unspecified)
                    }
                }.awaitAll()
            }

            popped.toSet() shouldBe expected.toSet()
            queue.peekNewestOrNull().shouldBeNull()
        }
...
private fun replacement(originalText: String): SuggestionReplacement = SuggestionReplacement(
    range = EditorRange(0, originalText.length),
    originalText = originalText,
    expectedSelection = EditorRange.cursor(originalText.length),
)

private fun content(text: String, selection: EditorRange): EditorContent = EditorContent(
    text = text,
    offset = 0,
...
private class TestCandidate(override val text: CharSequence, override val replacement: SuggestionReplacement) :
