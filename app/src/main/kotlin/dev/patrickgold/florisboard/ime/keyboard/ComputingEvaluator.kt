...
fun ComputingEvaluator.computeLabel(data: KeyData): String? {
    ...
    return if (data.type == KeyType.CHARACTER && data.code != KeyCode.SPACE && data.code != KeyCode.CJK_SPACE
        ...
        when (data.code) {
            ...
            KeyCode.VIEW_CHARACTERS -> evaluator.context()?.getString(R.string.key__view_characters)
            KeyCode.VIEW_NUMERIC,
            KeyCode.VIEW_NUMERIC_ADVANCED -> evaluator.context()?.getString(R.string.key__view_numeric)
            KeyCode.VIEW_PHONE -> evaluator.context()?.getString(R.string.key__view_phone)
            KeyCode.VIEW_PHONE2 -> evaluator.context()?.getString(R.string.key__view_phone2)
            KeyCode.VIEW_SYMBOLS -> evaluator.context()?.getString(R.string.key__view_symbols)
            KeyCode.VIEW_SYMBOLS2 -> evaluator.context()?.getString(R.string.key__view_symbols2)
            KeyCode.HALF_SPACE -> evaluator.context()?.getString(R.string.key__view_half_space)
            KeyCode.KESHIDA -> evaluator.context()?.getString(R.string.key__view_keshida)
            else -> null
        }
    }
}
...
fun ComputingEvaluator.computeImageVector(data: KeyData): ImageVector? {
    ...
    return when (data.code) {
        KeyCode.ARROW_LEFT -> Icons.AutoMirrored.Filled.KeyboardArrowLeft
        KeyCode.ARROW_RIGHT -> Icons.AutoMirrored.Filled.KeyboardArrowRight
        KeyCode.ARROW_UP -> Icons.Default.KeyboardArrowUp
        KeyCode.ARROW_DOWN -> Icons.Default.KeyboardArrowDown
        KeyCode.CLIPBOARD_COPY -> Icons.Default.ContentCopy
        KeyCode.CLIPBOARD_CUT -> Icons.Default.ContentCut
        KeyCode.CLIPBOARD_PASTE -> Icons.Default.ContentPasteGo
        KeyCode.CLIPBOARD_SELECT_ALL -> Icons.Default.SelectAll
        KeyCode.CLIPBOARD_CLEAR_PRIMARY_CLIP -> Icons.Default.DeleteSweep
        ...
        KeyCode.DELETE -> Icons.AutoMirrored.Outlined.Backspace
        KeyCode.ENTER -> {
        ...
        KeyCode.FORWARD_DELETE -> Icons.AutoMirrored.Default.ForwardDelete
        KeyCode.IME_UI_MODE_MEDIA -> Icons.Default.SentimentSatisfiedAlt
        KeyCode.IME_UI_MODE_CLIPBOARD -> Icons.AutoMirrored.Outlined.Assignment
        KeyCode.LANGUAGE_SWITCH -> Icons.Default.Language
        KeyCode.SETTINGS -> Icons.Default.Settings
        KeyCode.SHIFT -> {
        ...
        KeyCode.UNDO -> Icons.AutoMirrored.Filled.Undo
        KeyCode.REDO -> Icons.AutoMirrored.Filled.Redo
        KeyCode.TOGGLE_ACTIONS_OVERFLOW -> Icons.Default.MoreHoriz
        ...
        KeyCode.CHAR_WIDTH_FULL -> this.context()?.vectorResource(R.drawable.ic_keyboard_char_width_switcher_full)
        KeyCode.CHAR_WIDTH_HALF -> this.context()?.vectorResource(R.drawable.ic_keyboard_char_width_switcher_half)
        KeyCode.DRAG_MARKER -> {
            if (evaluator.state.debugShowDragAndDropHelpers) Icons.Default.Close else null
        }
        KeyCode.NOOP -> Icons.Default.Close
        else -> null
    }
}
