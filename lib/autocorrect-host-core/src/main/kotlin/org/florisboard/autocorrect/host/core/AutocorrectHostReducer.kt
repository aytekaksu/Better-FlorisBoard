...
private fun HostReduction.handle(event: HostEvent) {
    when (event) {
        ...
        is HostEvent.SetUiBindingDemand -> setUiBindingDemand(event)
        is HostEvent.OpenSession -> openSession(event)
        HostEvent.InvalidateEditor -> invalidateEditor()
        HostEvent.CloseSession -> closeSession()
        ...
        is HostEvent.SessionStartSending -> sessionStartSending(event)
        ...
        is HostEvent.FinishSendFailed -> finishSendFailed(event)
